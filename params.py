...
randomTxpm = False              # whether txpm should be randomized (untested)
...
if randomTxpm:
    ...
    labels = ["read-only",      # labels for the nodes (for plotting)
              "send-only",
              "average",
              "receive-only",
              "idle"]
...
chain = {
    ...
    "all" : {                                           # these parameters are shared across nodes
        "CHAINNAME": "chain1",
        "NETWORK_PORT": 7557,
        "RPC_PORT": 8002,
...
# generate header from username and password, DON'T CHANGE THIS
